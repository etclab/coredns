#!/bin/bash
# Config 6: CODoH with MLE ciphertext-only cache
# Architecture: Same as Config 4 (3-process IPC)
# Adds: MLE cache where enclave stores only encrypted entries (cannot read plaintext)

CONFIG_NAME="codoh-mle"
CONFIG_NUM=6
...
start_config() {
    ...
    # Start enclave (simulation mode, LRU + MLE cache)
    echo "Starting enclave (simulation, MLE cache)..."
    local secret_hex
